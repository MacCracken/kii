...
- **JPEG + non-PNG formats — via chitra** (re-pin, not an in-repo decoder) — superseded the old "in-repo JFIF at v1.2.0" plan. Post-re-fold kii consumes formats from the substrate: **chitra 0.2.1** adds sub-byte depths 1/2/4 + Adam7, **chitra 0.3+** adds JPEG. kii picks them up on a `[deps.chitra]` re-pin. Prefer chitra over any in-repo decoder (retires the in-repo JPEG/GIF/BMP line). See [ADR 0006](../adr/0006-adopt-chitra-decoder.md).
