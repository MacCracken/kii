...
- **Adopt `chitra` (PNG-substrate extraction — second consumer surfaced)** — the extraction has happened: **`chitra` 0.1.0 shipped 2026-06-19**, forked from kii's `src/png.cyr` core by the mabda agent so **mabda** could consume it (`gpu_texture_load_png`, a `[deps.chitra]` dist dep). kii's side is now the remaining step — switch to a `[deps.chitra]` dep and **drop the in-repo `src/png.cyr`**, closing the second-consumer loop (the `mihi → iam/chakshu` extract-on-2nd-consumer pattern, completed). **Gate — capability parity before cutover**: chitra 0.1.0 is PNG depth-8 / non-interlaced only; kii must not regress what its own decoder handles today (chitra 0.2 adds depth 1/2/4/16 + Adam7; chitra **0.3+ adds JPEG**, which subsumes kii's own JPEG/GIF/BMP plans above — prefer consuming chitra over an in-repo JFIF decoder). Opportunistic, no version gate. See [ADR 0001](../adr/0001-png-decoder-in-repo.md). *(Mirrored on the ecosystem roadmap in `agnosticos/docs/development/roadmap.md` § Parallel cycle work.)*
