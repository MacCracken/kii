...
**1.5.1** — cut 2026-08-25. **P-1 audit / refactor / hardening / optimization /
security sweep.** Five audit lenses (correctness, security, refactor, optimization,
docs + deferred-item) over the full surface, every material finding put through an
adversarial refutation pass: **41 raised, 2 refuted, 39 folded into 16 work items**.
Rendering is **byte-identical** at the default tier (22/22 golden artifacts).

The load-bearing finds: an unbounded `--width` wrapped `dst_w * dst_h * 3` into a
12-byte allocation the resampler then wrote past — **a reproducible SIGSEGV and
heap corruption on both render lanes**, now capped at the CLI and guarded inside
`downscale_to_rgb`; a truncated JPEG **rendered silently** because the `seen_iend`
gate had no JPEG arm and its comment wrongly claimed chitra forces the flag to 1;
the **1,000,000-iteration PNG fuzz surface had been testing nothing** since its
second iteration, because `alloc_reset()` invalidated sankoch's cached CRC-32
table; the ANSI-injection sanitizer covered C0 but **not C1**, so a filename built
from 8-bit control introducers delivered a complete OSC window-title injection to
stderr byte-for-byte; every `write(2)` to stdout was unchecked, so `kii img > out`
on a full filesystem exited **0** having written nothing; and `--color 8` had been
parsed, range-validated and then silently ignored since M1. Also: `CHITRA_ERR_UNSUPPORTED`
no longer routes a JPEG to the PNG § 11.2.2 bit-depth message, a 1-row terminal no
longer discards its own valid column count, and a fixture-guarded benchmark now
reports a visible SKIP instead of vanishing from the output. **550 assertions**
(was 511). See [`docs/audit/2026-08-25-audit.md`](../audit/2026-08-25-audit.md).
`print_version` → `kii 1.5.1`.
...
- `src/png.cyr` — **the chitra image adapter** (post-re-fold; was the 813-line native decoder). Decodes all four formats: `kii_decode_png` slurps under a 256 MB cap, sniffs the signature four ways (PNG→JPEG→BMP→GIF, matching chitra's own routing order), calls `chitra_image_decode`, and writes the canonical RGBA8 into the pstruct as a depth-8 ct6 image. Owns the `PNG_ERR_*` code space (16 codes), the 20-slot `STRUCT_*_OFFSET` layout (160 bytes; `SRC_COLOR_TYPE`=144, `FORMAT`=152), `KII_FMT_*`, `_png_color_channels` / `png_color_type_name` (every PNG colour type plus the JPEG `0x100|ncomp`, BMP `0x200|bpp` and GIF `0x300|min_code_size` sentinels), `kii_file_size` (lseek SEEK_END), and `_kii_map_chitra_err` (all 34 `ChitraErr` codes). Every parser and security guard — signature/IHDR/CRC/inflate/unfilter/PLTE, JPEG markers/Huffman/IDCT, BMP DIB/RLE/BITFIELDS, GIF LZW — lives in chitra. Historical `png_` / `PNG_ERR_*` / filename names retained; the rename to `image.*` / `IMG_ERR_*` is tracked and deferred (see the v1.5.1 audit § 5).
...
Build: **668 KB** on host at v1.5.1 (`build/kii` = 683,576 bytes, unstripped, DCE off). The `~145 KB` this section reported was measured at v0.8.0 and never refreshed across the cmdit fold, the chitra re-fold, or the JPEG/BMP/GIF folds — it was low by **4.6×**. Most of the size is the vendored `chitra` bundle carrying four decoders plus `sankoch`; the compiler reports ~942 unreachable fns (~390 KB) eliminable with `CYRIUS_DCE=1`. kii gained no module at v1.5.0 or v1.5.1. (Corrected at the v1.5.1 P-1 sweep.)

## Tests + bench

- `cyrius test` → **550 assertions, all pass** across 5 split suites (cli 77 / quant 125 / render 164 / ascii 17 / decode 167). v1.5.1 added 39 (P-1 regressions: C1 sanitizer coverage in both encodings, the `--color` tier contract, the downscale overflow guard, and the `_emit_bg_256_buf` negative-pos guard). At v1.5.0 decode grew 105 → 166 (+61: BMP + GIF e2e against ImageMagick-verified fixtures, the eleven BMP/GIF/budget error mappings, the nine new source-sentinel names plus four range-boundary guards, and a non-image UNKNOWN-tag case) and render grew 137 → 156 (+19: BMP and GIF decode→downscale→quantize→emit e2e, including a real content check that the GIF's pure-red and pure-lime quadrants quantize to palette entries 1 and 2 — the NORMAL entries, not the bright ones, because nearest-RGB puts pure (255,0,0) at 85² from entry 1 but 85²+85² from entry 9). **`tests/quant.tcyr`'s 109 assertions run again for the first time since v1.2.0** — 48 palette calls passed two arguments to the three-argument `assert_eq(a, b, name)`, so the suite had been failing to compile; the v1.4.1 cut missed it by verifying only the render suite. decode grew 51 → 105 at v1.4.0 (+54 JPEG: 11 error mappings + sentinel names + format tag + grayscale & YCbCr e2e with pixel/chroma/row checks + progressive/malformed stubs); render +8 (a JPEG decode→downscale→quantize→emit e2e). Decoder-internal coverage lives in chitra's suite.
- Fuzz: `cyrius build tests/kii.fcyr build/kii-fuzz && ./build/kii-fuzz` → **6,011,000 iters, all clean** (peak RSS **~136 MB**). **The PNG surface only became real at v1.5.1**: the per-iteration `alloc_reset()` that bounds the heap also invalidated sankoch's cached CRC-32 table (a lazy-init singleton holding a heap pointer), so from iteration 2 onward 2999 of every 3000 inputs died at the signature check rather than reaching the chunk walker, inflate, unfilter or the PLTE bounds. Every `alloc_reset()` is now paired with `crc32_table = 0`; the measured return-code tally afterwards matches a no-reset run exactly. Eight surfaces: 10k arg-parser + 1M path-sanitizer + 1M geometry + 1k emit-pipeline + 1M PNG + 1M JPEG + **1M BMP** + **1M GIF**. Each decode surface prefixes the real format signature (`\x89PNG`, `\xFF\xD8`, `BM`, `GIF89a`) so the random payload actually reaches that decoder rather than bouncing off the sniff. All four decode surfaces call `alloc_reset()` to rewind the never-free bump allocator each iteration (a matched signature makes chitra allocate marker/header/palette scratch before the input fails) — without it the heap would grow unboundedly. The **GIF surface is the highest-value of the four**: LZW builds a dictionary from attacker-controlled codes, the classic GIF-decoder CVE class.
