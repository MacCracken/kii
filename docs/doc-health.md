...
|---|---|---|---|
| `README.md` | 1 | 2026-08-24 (v1.5.0) | **Supported input formats** is now a positive list of all four (PNG / baseline JPEG / BMP / GIF-first-frame) rather than a "not yet supported" apology; toolchain pin, assertion count (511) and fuzz line (8 surfaces / 6,011,000 iters) refreshed; chitra substrate bullet cites ADR 0009. NOTE: the "Status" block still carries pre-v1.1 drift — see debt note. |
...
| `docs/development/roadmap.md` | 1 | 2026-08-25 (v1.5.1) | v1.5.0 row added; new **Upstream: `sankoch` limitations** section (S-1 Huffman hot spot / S-2 16 MiB inflate cap / S-3 `crc32_table` singleton) with measurements and a re-verify-at-next-pin-bump trigger. |
