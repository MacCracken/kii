...
| `CHANGELOG.md` | 1 | 2026-05-22 | `[0.1.0]` entry; rolls per release. |
...
| `cyrius.cyml` | 1 | 2026-05-22 | Description filled; stdlib deps + toolchain pin set. |
| `docs/development/state.md` | 1 | 2026-05-22 | Initial scaffold-only state. **Bump per release.** |
...
| `tests/kii.tcyr` | 2 | 2026-05-22 (cyrius init scaffold) | 2 smoke assertions. Expand per milestone. |
| `tests/kii.bcyr` | 2 | 2026-05-22 (cyrius init scaffold) | Stub. **Wire at M4** (first benchmark = quantization latency). |
| `tests/kii.fcyr` | 2 | 2026-05-22 (cyrius init scaffold) | Stub. **Wire at M2** (PNG-decoder fuzz harness). |
| `.github/workflows/ci.yml` | 1 | 2026-05-22 (cyrius init template) | Default CI scaffold. Verify before first release. |
| `.github/workflows/release.yml` | 1 | 2026-05-22 (cyrius init template) | Default release scaffold. Verify before first release. |
| `LICENSE` | 1 | 2026-05-22 | GPL-3.0-only header (parity with bannermanor/hapi/etc.). |
| `.gitignore` | 2 | 2026-05-22 | Standard scaffold. |

## Debt summary at v0.1.0

- **Tier-1 docs**: all fresh. Zero debt.
- **Tier-2 docs**: 5 cyrius-init-template stubs (`adr/README`, `adr/template`, `architecture/README`, `guides/getting-started`, `examples/.gitkeep`). All flagged with backfill milestones.
