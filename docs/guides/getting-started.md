...
- `src/test.cyr` — top-level test entry referenced by `cyrius.cyml [build].test`. Add unit cases here or in `tests/kii.tcyr`.
- `tests/kii.tcyr` — primary test suite (`cyrius test` auto-discovers).
- `tests/kii.bcyr` — benchmarks (`cyrius bench`).
- `tests/kii.fcyr` — fuzz harness (`cyrius fuzz`).

## Adding a feature

1. Edit `src/main.cyr` (or add a new module and `include` it).
2. Add a test case to `tests/kii.tcyr`.
3. Run `cyrius test`.
